...
# ========== LLM provider config ==========
# Choose between:
#   - gpt4all  -> local server (default; no API key)
#   - groq     -> hosted Groq API (needs GROQ_API_KEY)
LLM_PROVIDER = os.getenv("LLM_PROVIDER", "gpt4all").strip().lower()

# Base URLs
GROQ_CHAT_URL = "https://api.groq.com/openai/v1/chat/completions"
GPT4ALL_BASE = os.getenv("LLM_BASE_URL", "http://localhost:4891/v1").rstrip("/")
GPT4ALL_CHAT_URL = f"{GPT4ALL_BASE}/chat/completions"

# Model name (confirm available models via GET {LLM_BASE_URL}/models)
# Example good low-RAM choice: "phi-3-mini-4k-instruct-q4_0"
...
def insights():
    \"\"\"Accepts either direct fields or {current, logs} and returns lifestyleCorrelations, etc.\"\"\"
    try:
...
def report():
    \"\"\"Build the five Report sections and return one JSON object.\"\"\"
    try:
        payload = request.get_json(silent=True) or {}
        ...
        # 1) Lifestyle correlations (reuse logic)
        lifestyleCorrelations = []
        ...
        # 2) Highlights (LLM with fallback)
