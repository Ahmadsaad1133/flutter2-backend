...
def _clean_json_str(s: str) -> str:
    """
    Loosely fix common JSON issues for LLM outputs:
    - strip code fences
    - prefer the largest {...} or [...] block
    - swap single -> double quotes (heuristic; skip http(s) urls)
    ...
    s = re.sub(r"(?:'[^']*')", lambda m_: repl_quotes(m_), s)
    s = re.sub(r",\s*([}\]])", r"\1", s)
    s = re.sub(r"//.*", "", s)
    return s.strip()
...
def _fetch_and_embed_image(path_or_url: str) -> Optional[str]:
    """
    Accepts http(s) URL, local path, or data URI. Returns a data URI (or None).
    """
    if not path_or_url:
        return None
...
# --------------------- Text Sanitizer (keep only human-readable text) ---------------------
def _clean_text(txt: Any) -> str:
    """
    Convert any JSON-ish / markdown-ish / noisy content into clean, plain text.
    - If looks like JSON (dict/list), flatten to 'k: v' pairs or bullets.
    - Strip braces/quotes and collapse whitespace.
    - Keep only readable text; caller may add <br/> after escaping.
    """
    if txt is None:
        return ""
    s = str(txt)

    # If likely JSON, try to parse and flatten nicely
    candidate = s.strip()
    if candidate.startswith("{") or candidate.startswith("["):
        try:
            parsed = json.loads(candidate)
            if isinstance(parsed, dict):
                # keep shallow dict readable
                return " ".join(f"{k}: {parsed[k]}" for k in parsed.keys())
            if isinstance(parsed, list):
                return " • ".join(str(x) for x in parsed)
        except Exception:
            pass

    # remove curly/array braces & quotes (avoid weird symbols)
    s = re.sub(r"[{}\[\]\"]", "", s)
    # collapse whitespace
    s = re.sub(r"\s+", " ", s)
    return s.strip()
...
def _svg_bar_chart(title: str, series: List[Dict[str, Any]], width: int = 700, height: int = 280) -> str:
    ...
    for i, s in enumerate(series):
        ...
        labels.append(
            f'<text x="{x + (bar_w-12)/2:.1f}" y="{y0 + chart_h + 16:.1f}" font-size="11" text-anchor="middle">{_escape(str(s.get("label","")))}</text>'
        )
    ...
    for gy in range(5):
        y = y0 + gy * (chart_h / 4)
        grid.append(
            f'<line x1="{x0}" y1="{y:.1f}" x2="{x0+chart_w}" y2="{y:.1f}" stroke="#ddd" stroke-dasharray="3,3" />'
        )
...
def _svg_donut_chart(title: str, items: List[Dict[str, Any]], width: int = 260, height: int = 260) -> str:
    ...
    for idx, item in enumerate(items):
        ...
        arcs.append(path)
        legend.append(
            f'<div style="display:flex;align-items:center;gap:8px;margin:2px 0;"><span style="display:inline-block;width:10px;height:10px;background:{color};border-radius:2px;"></span><span style="font-size:12px;color:#333">{_escape(str(item.get("label","")))} — {round(pct*100)}%</span></div>'
        )
...
def _groq_structured_analysis(user_payload: dict) -> dict:
    """
    When GROQ_API_KEY is present and auto_groq is True, transform the user payload to a
    strictly structured JSON we can render. Else, fall back to using user payload as-is.
    """
    if not GROQ_API_KEY:
        # No key: build context directly from provided payload, but add meta
        return {
            "meta": {
    ...
    body = {"model": GROQ_MODEL, "messages": [{"role": "system", "content": sys_prompt}, user_prompt], "temperature": 0.3}
...
def _render_html(ctx: dict) -> str:
    ...
    # Sanitize and escape overview; allow <br/> only
    overview_text = _clean_text(ctx.get("overview", ""))
    overview_safe = _escape(overview_text).replace("\n", "<br/>")

    # Sanitize lists
    insights = [_clean_text(i) for i in (ctx.get("insights") or [])]
    ...
    for ch in ctx.get("charts", []):
        ...
        title = _clean_text(ch.get("title") or "Chart")
        data = ch.get("data") or []
        if ctype == "bar":
    ...
    for img in ctx.get("images", []):
        ...
        if src:
            ...
            if data_uri:
                ...
                image_blocks.append(
                    f'<div class="card"><div class="small" style="margin-bottom:6px">{ttl}</div><img class="img" src="{data_uri}"/></div>'
                )

    # Sections
    section_blocks = []
    for sec in ctx.get("sections", []):
        title = _escape(_clean_text(sec.get("title", "Section")))
        body_txt = _clean_text(sec.get("body", ""))
        body_html = _escape(body_txt).replace("\n", "<br/>")
        section_blocks.append(
            f'<div class="card section"><div class="kicker">{title}</div><div style="margin-top:6px;font-size:13px;line-height:1.5">{body_html}</div></div>'
        )
      ...
      <body>
        ...
        <div class="section grid-2">
          <div class="card">
            ...
            <div style="margin-top:6px;font-size:13px;line-height:1.5">{{ overview_html|safe }}</div>
          </div>
          <div class="card">
...
def generate_pdf():
    ...
    Accepts JSON:
    {
      ...
      "sections": [{"title":"", "body":""}],
      "charts": [
        {"type":"bar","title":"...","data":[{"label":"Mon","value":64}]},
        {"type":"donut","title":"...","data":[{"label":"Deep","value":90}]},
        {"type":"line","title":"...","data":[{"label":"2025-09-01","value":72}]}
      ],
      "images": [{"title":"...","url_or_data_uri":"https://... or data:..."}],
      "metrics": {...},
      "auto_groq": true,
    ...
    auto_groq = bool(payload.get("auto_groq")) or not any(
        [payload.get("overview"), payload.get("insights"), payload.get("sections")]
    )
    ...
    # Render HTML (with sanitization inside)
    ...
    filename = payload.get("download_filename") or (
        ctx["meta"].get("title", "report").lower().replace(" ", "_") + ".pdf"
    )
...
def _demo():
    demo_ctx = {
        ...
        "charts": [
            {
                "type": "bar",
                "title": "Sleep Quality (last 7 days)",
                "data": [
                    {"label": "Mon", "value": 60},
                    {"label": "Tue", "value": 68},
                    {"label": "Wed", "value": 62},
            ...
            },
            {
                ...
                "title": "Latency (mins)",
                "data": [
                    {"label": "09-01", "value": 28},
                    {"label": "09-02", "value": 24},
                    {"label": "09-03", "value": 22},
                    {"label": "09-04", "value": 20},
                    {"label": "09-05", "value": 21},
                    {"label": "09-06", "value": 19},
                    {"label": "09-07", "value": 18},
                ],
            },
        ],
        "images": [
